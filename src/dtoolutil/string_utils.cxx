 ...
 */
int
extract_words(const wstring &str, std::vector<wstring> &words) {
  int num_words = 0;
 ...
 */
void
tokenize(const wstring &str, std::vector<wstring> &words,
         const wstring &delimiters, bool discard_repeated_delimiters) {
  size_t p = 0;
  while (p < str.length()) {
    size_t q = str.find_first_of(delimiters, p);
    if (q == string::npos) {
